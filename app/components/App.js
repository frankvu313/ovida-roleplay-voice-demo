...
import { AGENT_GREETING } from "app/lib/constants";
...
export const App = ({
  ...
  const stopConversation = () => {
    // Stop microphone from sending data
    if (processor) {
      processor.onaudioprocess = null;
    }
    ...
    // Put bot to sleep (this will stop listening and speaking)
    startSleeping();
  };
    ...
    <div className={className}>
      ...
      {!microphone ? (
        ...
        <Fragment>
          ...
          {socketState === 0 && (
            <div className="text-base text-gray-25 text-center w-full">Loading Ovida...</div>
          )}
          {socketState > 0 && status === VoiceBotStatus.SLEEPING && (
            <div className="text-xl flex flex-col items-center justify-center mt-4 mb-5 md:mt-4 md:mb-5">
              <div className="text-gray-450 text-sm">
                I&apos;ve stopped listening. {isMobile ? "Tap" : "Click"} the orb to resume.
              </div>
            </div>
          )}
          {socketState > 0 && status !== VoiceBotStatus.SLEEPING && status !== VoiceBotStatus.NONE && (
            <div className="flex justify-center mt-4 mb-5 md:mt-4 md:mb-5">
              <button
              ...
              >
                ...
                <span className="text-sm font-medium">Stop Conversation</span>
              </button>
            </div>
          )}
        </Fragment>
